...
// index_cesium.html. Cache name is namespaced so the two don't collide.
//
// Strategy:
...
const CORE = [
  ...
  ...['tz_lookup','format','state','tabs','cities','search_parser','eclipse',
      'search','list','local','details','share','map_cesium','url','init'].map(n => `js/${n}.js`),
...
self.addEventListener('fetch', e => {
  ...
  if (req.mode === 'navigate') {
    e.respondWith((async function () {
      var shell = function () {
        return caches.match('index_cesium.html', { cacheName: CACHE, ignoreSearch: true });
      };
