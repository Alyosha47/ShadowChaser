...
> **State:** all work below is DONE, VERIFIED, and committed-or-ready. Desktop is stable;
> there are no open desktop bugs blocking work.
> **The single next task is: add a service worker / PWA** so a true offline (no-signal)
> reload actually loads the app instead of Chrome's dinosaur. Everything it needs is in
> place (MapLibre + deck.gl are now vendored locally — see §"Foundation"). Read §"NEXT
> TASK — SERVICE WORKER" at the bottom; it has the full plan, the gotchas, and the one
> CSP caveat. You do NOT need to get up to speed on anything else to start it.
...
**Last updated:** 2026-05-31, end of a long, high-yield Opus 4.8 session (offline-globe
fixes, the V-angle math, far-side markers, oval blink-off, and vendoring MapLibre/deck.gl
locally).
...
`2026-05-30h`) and is appended as `?v=BUILD` to every `js/*` and `data/*` fetch. **Bump it
...
- **PWA / service worker** — keystone of the offline-in-the-field goal. **← THE NEXT TASK;
  full plan at the bottom of this doc.**
...
Replace files → **bump `BUILD` in index.html** → hard-refresh → verify on server
(`chmod 755` dirs / `644` files if you get 403s) → on a real device, sanity-check that the
local globe shows and a map click doesn't error. Vendored files in `vendor/` don't take
`?v=BUILD` (version is in the filename).

---

## ▶ NEXT TASK — SERVICE WORKER / PWA (full plan, start here)

**Goal:** a true offline (no-signal) reload loads the app from cache instead of Chrome's
offline dinosaur. This is THE last foundation piece for "works in the field." Vendoring
(done) was the prerequisite — every runtime asset is now same-origin and cacheable.

**Why it's needed (the dinosaur):** the offline throttle blocks the very first request
(`index.html`), so nothing runs. Only a service worker, which serves `index.html` + assets
from the Cache Storage API, fixes that. Nothing else can.

**Scope / plan:**
1. `sw.js` at repo root (scope = site root). Register from `index.html` (or `init.js`)
   behind `if ('serviceWorker' in navigator)`.
2. **Precache** on `install`: `index.html`, `css/app.css`, all `js/*.js`, all `vendor/*`,
   `fonts/*`, and the basemap set under `data/basemap/*.gz`. Decide whether to precache the
   besselian century JSONs (large — maybe cache-on-demand via runtime caching instead).
3. **Fetch strategy:** cache-first for the app shell + vendor + fonts; for `data/`,
   stale-while-revalidate or cache-on-demand (the besselian files are big and rarely all
   needed). Online raster tiles (the online-upgrade basemap) must remain network-only —
   never cache those.
4. **Cache versioning:** key the cache name to a version string and delete old caches on
   `activate`. **Reuse the existing `BUILD` value** so there is ONE version concept, not
   two — otherwise the SW becomes a second stale-cache trap on top of `?v=BUILD`. (This is
   the single biggest risk: a careless SW makes "why didn't my change show up?" permanent.
   Plan the update/skipWaiting/clients.claim flow deliberately.)
5. **Add a web app manifest** (`manifest.webmanifest`): name, icons, `display:standalone`,
   theme/background color → enables real install + the splash item in BACKLOG features.
   (There is currently NO manifest and only a `favicon.ico`.)

**⚠ The one gotcha that will silently break the map:** MapLibre's worker is an inline
**Blob** (`blob:` URL). If you add a Content-Security-Policy with the SW, you MUST allow
`worker-src blob:` and `script-src blob:`, or switch to MapLibre's `maplibre-gl-csp-worker.js`
+ `maplibregl.setWorkerUrl()`. (See Foundation section.) Test the map specifically after
the SW lands.

**How to test properly:** (a) load online once so the SW installs and precaches; (b) THEN
set DevTools→Network→Offline and reload — the app should load and the globe render. Also
check Application→Service Workers (registered/activated) and Application→Cache Storage
(assets present). The Network "no unpkg" check from vendoring still applies.

**Definition of done:** offline reload loads the app and renders the offline globe; a new
deploy (BUILD bump) reliably replaces the old cache (no stale assets); map still draws
(Blob-worker/CSP ok).

---

## AFTER THE SERVICE WORKER — the frontier
