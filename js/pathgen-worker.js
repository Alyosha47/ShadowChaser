/* pathgen-worker.js — Follow the Shadow — https://followtheshadow.com
   ...
   Message in:  { id, rec, what }   rec: one Besselian record; what: 'mag' for the
                                    equal-magnitude curves, otherwise the path
...
self.onmessage = function (e) {
  var m = e.data;
  try {
    self.postMessage({ id: m.id, path: m.what === 'mag' ? PathGen.eclipse_magnitude_curves(m.rec)
                                                         : PathGen.eclipse_path(m.rec) });
  }
