...
└── js/
    ...
    ├── search.js       (parseCoords, onSearchChanged; textarea autogrow is done by field-sizing CSS)
...
- Antimeridian camera elegant single-path; **V-angle math (validated both hemispheres)**;
  ...
  click, via `isLocationOccluded` (was #R2)**. Polish: tab/chrome contrast; Rise/Set
  labels; corona brightness; search-instruction layout + canonical "Obscuration";
  search-range setting; share rewrite; About mailto + Android note.
...
**Where things stand:** the **desktop real-bug tier is clear.** Remaining work is
(a) mobile bugs (#R4 offline basemap, #R5 pinch-zoom) — parked until desktop is stable
per the user's standing rule; (b) deferred polish (#R1 city-label fade, dropped-pin icon,
server-side share page, etc.); and (c) the big features. The natural next frontier is
**feature scoping (Opus-grade): #F1 personal log, #F2 weather/cloud-cover overlay,
#F3 animated shadow.** #F2 is the killer feature but ~2 sessions of design before code.
Confirm direction with the user before starting — these involve UX and data-source
decisions that are explicitly the user's to make.
