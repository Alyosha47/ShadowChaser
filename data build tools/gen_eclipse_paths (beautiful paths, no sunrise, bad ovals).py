...
def umbra_ovals(rec, oval_step_min=OVAL_STEP_MIN, N=24):
...
def build_path(rec, step_min=STEP_MIN, pen_n=PEN_N):
    ...
    # ── Douglas-Peucker simplification ─────────────────────────────────
    # Per-curve tolerances reflect the physics, not a uniform accuracy bar:
    ...
    for fld in ('centreline', 'umbra_n', 'umbra_s'):
...
def process_chunk(path, out_dir, step_min, pen_n):
    ...
    for rec in records:
        ...
        paths[key]=_round_coords(build_path(rec, step_min, pen_n))
